...
void J1939_TP_tx::initialize(uint8_t port)
{
  tx_mbox.initialize();
  ...
  rx_mbox.rx_id.set_J1939ID(0, 0xEC, 0xFF, 0xFE);   // initially only receive from the NULL address
  rx_mbox.rx_id_mask.set_J1939ID(0, 0xFF, 0xFF, 0xFF);
...
void J1939_TP_tx::update(void)
{
  ...
  if (rx_mbox.msg_age_mS < 60000)
  {
    ...
    case CM_EndOfMsgACK:
      ...
      rx_mbox.rx_id.set_J1939_SA(0xFE);
  ...
  case TRANSMITTING_DT:
    ...
    if (tx_mbox.timer.off())
    {
      if (packets_to_send > 0)
      {
        ...
        tx_mbox.mbuff.id.set_J1939ID(7, 0xEB, jmsg->id.get_J1939_DA(), jmsg->id.get_J1939_SA());
        tx_mbox.mbuff.DLR = 8;
...
void J1939_TP_tx::SendMsg(J1939_msg_t *jmsg)
{
  if (Idle())
  {
    this->jmsg = jmsg;
    ...
    {   // send a CM message to initiate this tranmsmit protocol session...
      uint8_t sa = jmsg->id.get_J1939_SA();
      rx_mbox.rx_id.set_J1939_DA(sa);        // only receive CM dessages directed at this source address

      uint8_t da = jmsg->id.get_J1939_DA();
      rx_mbox.rx_id.set_J1939_SA(da);   // and from the SA the message is being sent to...
      ...
      mptr->id.set_J1939ID(7, 0xEC, da, sa);
      ...
      uint16_t pgn = jmsg->id.get_J1939_PGN();
