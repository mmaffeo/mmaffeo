...
static void InitWatchdog(void)
{
// The Watchdog Low Speed Internal Oscilator can vary from 30khz to 60khz
// At a div_by_8 prescaler, a 2250 reload gives a ~300ms to ~600ms watch dog period
//  (2250) / (30,000 / 8) = ~0.600 seconds
//  (2250) / (60,000 / 8) = ~0.300 seconds
#define IWDOG_RELOAD_VALUE  (2250)
#define IWDKR_KEY_RELOAD    ((uint16_t)0xAAAA)
#define IWDKR_KEY_ENABLE    ((uint16_t)0xCCCC)

  // TODO maybe disable this for release build
  // Stop the watchdog clock when Core halted by JTAG
  DBGMCU->APB1FZ |= DBGMCU_IWDG_STOP;

  // Enable the Watchdog
  IWDG->KR = IWDKR_KEY_ENABLE;

  // Enable write access to IWDG Relaod and Prescaler
  ...
  // IWDG Set Reload
  IWDG->RLR = IWDOG_RELOAD_VALUE;

  // Disable write access to IWDG Relaod and Prescaler
  IWDG->KR = IWDG_WriteAccess_Disable;

  // Wait until both regitsters update
  while (IWDG->SR != 0);

  // Trigger the first IWDG reload
  IWDG->KR = IWDKR_KEY_RELOAD;
}
...
#if defined (STM32F40_41xxx) || defined (STM32F427_437xx) || defined (STM32F429_439xx)
    /* PCLK2 = HCLK / 2  (APB2 Prescaler) */
    RCC->CFGR |= RCC_CFGR_PPRE2_DIV2;

    /* PCLK1 = HCLK / 4  (APB1 Prescaler) */
    RCC->CFGR |= RCC_CFGR_PPRE1_DIV4;
#endif /* STM32F40_41xxx || STM32F427_437x || STM32F429_439xx */

#if defined (STM32F401xx)
    /* PCLK2 = HCLK / 1  (APB2 Prescaler) */
